...
exports.toExportCsvRow = toExportCsvRow;
...
const MEMBER_COLUMNS = ['id', 'email', 'name', 'note', 'status', 'created_at'];
function namesToCsv(items) {
...
class MembersCSVExporter {
    _knex;
    _members;
    _metafields;
    constructor({ knex, members, metafields }) {
        this._knex = knex;
    ...
    async export(options = {}) {
        ...
        if (ids) {
            logging.info({ event: { name: 'members-export.filtered' }, matched: ids.length }, 'Found members matching the export filter');
        }
        ...
        logging.info({ event: { name: 'members-export.started' } }, 'Starting streaming export of members');
        const batchingTransform = this.createBatchingTransform();
        const processingTransform = this.createProcessingTransform(reference);
        (0, node_stream_1.pipeline)(membersQuery.stream(), batchingTransform, processingTransform, (err) => {
            if (err) {
                logging.error({ event: { name: 'members-export.stream.error' }, err }, 'Members export stream failed');
            }
            else {
                logging.info({ event: { name: 'members-export.completed' }, durationMs: Date.now() - start }, 'Members export completed');
            }
        });
    ...
    async fetchReferenceData() {
        ...
        logging.info({ event: { name: 'members-export.reference_data_read' }, durationMs: Date.now() - start }, 'Read the products and labels an export names');
    ...
    createProcessingTransform(reference) {
        const assembleRelatedData = (memberIds) => this.assembleRelatedData(memberIds, reference.activeMetafields);
        const flattenBatch = (members, related) => this.flattenBatch(members, related, reference);
        return new node_stream_1.Transform({
            objectMode: true,
    ...
    async assembleRelatedData(memberIds, activeMetafields) {
        const knex = this._knex;
        const [tiers, labels, stripeCustomers, subscriptions, gifts, metafieldValuesMap] = await Promise.all([
        ...
        return {
            tiersMap: new Map(tiers.map((row) => [row.member_id, row.tiers])),
            labelsMap: new Map(labels.map((row) => [row.member_id, row.labels])),
            stripeCustomerMap: new Map(stripeCustomers.map((row) => [
                row.member_id,
                row.stripe_customer_id,
            ])),
            subscribedSet: new Set(subscriptions.map((row) => row.member_id)),
            giftIdMap: new Map(gifts.map((row) => [
    ...
    flattenBatch(members, related, reference) {
        const { tiersMap, labelsMap, stripeCustomerMap, subscribedSet, giftIdMap, metafieldValuesMap } = related;
        ...
        return members.map((row) => {
            ...
            return {
                ...
                created_at: (0, moment_1.default)(row.created_at).toISOString(),
...
exports.default = MembersCSVExporter;
