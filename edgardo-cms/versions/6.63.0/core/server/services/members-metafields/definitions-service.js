...
const knexify = require('@tryghost/mongo-knex');
...
class MetafieldDefinitionsService {
    ...
    async browse(options = {}, audience = access_1.ADMIN) {
        ...
        const query = options.filter
            ...
            : (0, queries_1.activeFields)(this.knex);
        return (0, access_1.readableFields)(audience, await this.list(query));
    }
    /**
...
function applyFilter(query, filter) {
    ...
    if (!filterReferencesStatus(mongoQuery)) {
        query.where('status', schema_1.FIELD_STATUS.active);
    }
    return query;
}
