...
exports.resolveMaxDefinitions = resolveMaxDefinitions;
...
const Ceiling = zod_1.z
    .union([zod_1.z.number(), zod_1.z.string().trim().min(1)])
    .transform(Number)
    .pipe(zod_1.z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER));
...
const shippedCeiling = Ceiling.parse(require('../../../shared/config/defaults.json').members.metafields.maxDefinitions);
...
function resolveMaxDefinitions(configured) {
