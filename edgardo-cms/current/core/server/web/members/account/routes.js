...
module.exports = function accountRoutes() {
    ...
    if (config.get('cacheMembersContent:enabled')) {
        ...
        middleware.loadMemberSession, middleware.accessInfoSession, middleware.emptyWhenAnonymous, http(api.membersAccount.read));
    }
    else {
        router.get('/', middleware.emptyWhenAnonymous, http(api.membersAccount.read));
    }
    router.put('/', bodyParser.json({ limit: '50mb' }), middleware.rejectWhenAnonymous, http(api.membersAccount.update));
    ...
    router.get('/metafields/:namespace', middleware.rejectWhenAnonymous, http(api.memberMetafieldsMembers.browse));
    // Letting Ghost email this member again after it stopped.
    router.delete('/suppression', middleware.rejectWhenAnonymous, http(api.membersAccount.destroySuppression));
    return router;
};
